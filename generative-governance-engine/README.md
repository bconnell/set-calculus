...
The current `run()` API exposes a default `max_steps = 100` execution bound. Execution-budget exhaustion is not itself a mathematical Resolution state. After the last permitted Transform, the engine observes the resulting State and resolves that State under the normal resolution algebra. If work remains feasible, the result stays `INCOMPLETE` and the trace records `max steps exhausted`.
