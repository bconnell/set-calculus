...
Set Calculus distinguishes the historical source of an observed pattern from the generalized structure derived from that pattern.

The historical name **Collatz** is retained as provenance for the source problem and its observed dynamics.

The reusable structural term is:

# Triadic Resolution Path

This naming separates provenance from authority.

```text
Collatz
= historical source / provenance anchor

Triadic Resolution Path
= generalized structural abstraction
```
...
The Triadic Resolution Path describes the generalized three-path resolution structure:

```text
lower-bound path
middle / balancing path
upper-bound path
```
